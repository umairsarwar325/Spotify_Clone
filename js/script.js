...
async function getFolders() {
  ...
  let getF = await fetch(
    `http://127.0.0.1:3000/web%20development/01Practice/Spotify_Clone_Sigma_CWH/Songs/`
  );
  ...
  for (let index = 0; index < allFolders.length; index++) {
    ...
    console.log(elem.split("/").slice(-2)[0]);
...
async function getSongs() {
  ...
  let data = await fetch(
    `http://127.0.0.1:3000/web%20development/01Practice/Spotify_Clone_Sigma_CWH/Songs/${folder}/`
  );
...
function playSong(oneSong) {
  currentSong.src = `http://127.0.0.1:3000/web%20development/01Practice/Spotify_Clone_Sigma_CWH/Songs/${folder}/${oneSong}.mp3`;
  if (playing) {
    playMusic();
  }
  else{
